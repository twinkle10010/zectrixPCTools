...
use windows::Win32::UI::WindowsAndMessaging::{
    FindWindowW, GetMessageW, GetWindowThreadProcessId, ShowWindow,
    SW_RESTORE, SW_SHOW, SW_HIDE, IsWindowVisible, IsIconic, SetWindowPos,
...
// 防抖：热键触发后忽略短暂内的重复触发
static TRIGGER_LOCK: AtomicBool = AtomicBool::new(false);
    ...
    if let Some(hwnd) = find_main_window() {
        unsafe {
            ...
            if is_visible && !is_minimized {
                ...
                // 用 SetWindowPos 尝试激活（NOACTIVATE 避免 beep，失败了就算了）
                let _ = SetWindowPos(hwnd, HWND::default(), 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE | SWP_SHOWWINDOW);
...
fn main() {
    ...
    std::thread::spawn(move || {
        ...
        if let Err(e) = listen(move |event: Event| {
            ...
            if has_alt && has_c {
                // 原子CAS：确保只有一个线程处理本次触发
                if !TRIGGER_LOCK.compare_exchange(false, true, Ordering::Acquire, Ordering::Relaxed).is_ok() {
                    current_keys.clear();
                    return;
                }
                ...
                std::thread::spawn(|| {
                    ...
                    TRIGGER_LOCK.store(false, Ordering::Release);
                });
    ...
    println!("Hotkey extension ready. Press Alt+C to toggle window, right-click to quit.");
